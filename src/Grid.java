...
public class Grid {
	/**
	 * Maximum x coordinate used by constructor, recalc, neighbor
	 **/
	public static int bigX;
	/**
	 * Maximum y coordinate used by constructor, recalc, neighbor
	 */
	 ...
	 * 
	 * @param lengthX
	 *            , lengthy are the size of the array
	 */
	...
	void recalc() {
		...
		while (!list.isEmpty()) {
			...
			for (int direction = 0; direction < 4; direction++) { // iterate
																	// over
																	// neighbors
	 ...
	 *            of the neighbor
	 */
	private Node neighbor(Node aNode, int direction) {
		Node node = aNode;
		Node neighbor = null;
		direction %= 4;
		int x = node.getX();
